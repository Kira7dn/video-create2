...
class ExpansionConfig:
    ...
    max_expand_s: float = 0.250  # Maximum expansion per side (seconds) when no neighbor; with pad_s=0.08 → 0.33 (matches tests)
    ...
    max_internal_gap_s: float = 0.950  # Bridge sub-second internal gaps, but avoid crossing inter-line pauses (~>1s)
...
class TextOverBuilder(ITextOverBuilder):
    ...
    def __init__(self, *, temp_dir: str, config: Optional[ExpansionConfig] = None) -> None:
    ...
    def build(
        ...
        *,
        ...
        try:
            self._success_index_map = {id(w): idx for idx, w in enumerate(success_words)}
        except Exception:
            self._success_index_map = {}
        try:
            self._success_token_sets = [
                set(normalize_text(str(w.get("word", "")))) if isinstance(w, dict) else set()
        ...
        for chunk in chunks:
            ...
            item = self._process_chunk(chunk, success_words, word_index, media_duration_s)
            if item:
                text_over_items.append(item)
                word_index = self._advance_word_index(item.get('_matched_group', []), success_words, word_index)
    ...
    def _find_alignment(
        ...
        window_items = success_words[word_index : word_index + self._config.lookahead_window]
        flex_group = find_flexible_match(
            normalized_text, 
            window_items, 
            max_lookahead=self._config.lookahead_window
        )
        
        return flex_group or []

    def _extend_group_internal(self, group: List[Dict[str, Any]], success_words: List[Dict[str, Any]], normalized_chunk_tokens: List[str]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            current_start = min((w["start"] for w in group if isinstance(w, dict) and "start" in w), default=None)
            current_end = max((w["end"] for w in group if isinstance(w, dict) and "end" in w), default=None)
            ...
            def _bridge(direction: int) -> None:
                ...
                while bound_check(idx):
                    ...
                    if direction > 0:
                        ...
                        w_tokens_set = self._success_token_sets[idx] if 0 <= idx < len(self._success_token_sets) else set()
    ...
    def _apply_expansion(
        ...
        left_idx, right_idx = self._find_group_indices(group, success_words)
        prev_end, next_start = self._get_neighbor_times(left_idx, right_idx, success_words)
        ...
        is_right_extreme = bool(right_idx is not None and right_idx == len(success_words) - 1)
        ...
        extend_left = self._calculate_side_extension(left_gap, is_left=True, allow_no_neighbor=is_left_extreme)
        extend_right = self._calculate_side_extension(right_gap, is_left=False, allow_no_neighbor=is_right_extreme)
    ...
    def _get_base_timing(self, item: Dict[str, Any], group: List[Dict[str, Any]]) -> Tuple[float, float]:
    ...
    def _find_group_indices(self, group: List[Dict[str, Any]], success_words: List[Dict[str, Any]]) -> Tuple[Optional[int], Optional[int]]:
        ...
        word_to_index = self._success_index_map or {id(word): idx for idx, word in enumerate(success_words)}
            ...
            for word in group:
                ...
                if idx_id is not None:
                    ...
                    for idx, sw in enumerate(success_words):
                        ...
                        logger.debug(f"Word not found in success_words: {word.get('word', 'unknown')}")
    ...
    def _get_neighbor_times(self, left_idx: Optional[int], right_idx: Optional[int], success_words: List[Dict[str, Any]]) -> Tuple[Optional[float], Optional[float]]:
        ...
        try:
            ...
            if left_idx is not None and 0 <= left_idx < len(success_words) and left_idx > 0:
    ...
    def _calculate_side_extension(self, gap: Optional[float], is_left: bool, allow_no_neighbor: bool) -> float:
    ...
    def _advance_word_index(self, group: List[Dict[str, Any]], success_words: List[Dict[str, Any]], current_index: int) -> int:
        ...
        try:
            ...
            indices: List[int] = []
            for item in group:
                idx_id = self._success_index_map.get(id(item)) if self._success_index_map else None
                if idx_id is not None:
    ...
    def _normalize_total_duration(self, items: List[Dict[str, Any]], success_words: List[Dict[str, Any]]) -> None:
        ...
        def cap_extreme_left(it: Dict[str, Any], cap: float) -> None:
            ...
            base_start = float(it.get("_base_start", start))
            desired_start = min(base_start, start + cap)  # cannot extend left more than cap beyond base_start
        ...
        def cap_extreme_right(it: Dict[str, Any], cap: float) -> None:
            ...
            base_end = float(it.get("_base_end", end))
            desired_end = max(base_end, end - cap)  # cannot extend right more than cap beyond base_end
    ...
    def _persist_output(self, text_over_items: List[Dict[str, Any]], text_over_id: str) -> None:
        ...
        try:
            ...
            clean_items = [{k: v for k, v in item.items() if not k.startswith('_')} for item in text_over_items]
            ...
            logger.info(f"Persisted {len(clean_items)} text overlay items to {out_path}")
        except Exception as e:
